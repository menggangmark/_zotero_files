{
  "translatorID": "9b85ff96-ceb3-4ca2-87a9-154c18ab38b1",
  "label": "Better BibTeX Quick Copy",
...
// SOURCE: resource/translators/Better BibTeX Quick Copy.header.js

      Translator.header = {"translatorID":"9b85ff96-ceb3-4ca2-87a9-154c18ab38b1","label":"Better BibTeX Quick Copy","creator":"Emiliano heyns","target":"txt","minVersion":"4.0.27","maxVersion":"","displayOptions":null,"translatorType":2,"browserSupport":"gcsv","priority":100,"inRepository":true};
      Translator.release = "1.6.75";
      Translator.BetterBibTeXQuickCopy = true;
    

// SOURCE: resource/translators/Better BibTeX Quick Copy.js
// Generated by CoffeeScript 1.10.0
var Mode, doExport;
...
Mode = {
  ...
  latex: function() {
    ...
    if (cmd === '') {
      return Zotero.write(keys.join(','));
    } else {
      return Zotero.write("\\" + cmd + "{" + (keys.join(',')) + "}");
    }
  },
  citekeys: function() {
    var item, keys;
  ...
  orgmode: function() {
    var groupID, id, item, key, m, results, type;
    results = [];
    while (item = Translator.nextItem()) {
      ...
      switch (type) {
        case 'users':
          if (groupID.indexOf('local') !== 0) {
            Translator.debug("Link to synced item " + item.uri);
          }
          id = "0_" + key;
          break;
        case 'groups':
      ...
      results.push(Zotero.write("[[zotero://select/items/" + id + "][@" + item.__citekey__ + "]]"));
    }
    return results;
  }
};
